...
export class FollowEffects extends BaseLoopbackEffects {
  ...
  public getUser$ = this.actions$
    .ofType(FollowActionTypes.GET_USER)
    .mergeMap((action: LoopbackAction) =>
      this.follow.getUser(action.payload.id, action.payload.refresh)
  ...
  public getFollowee$ = this.actions$
    .ofType(FollowActionTypes.GET_FOLLOWEE)
    .mergeMap((action: LoopbackAction) =>
      this.follow.getFollowee(action.payload.id, action.payload.refresh)
        .mergeMap((response: any) => concat(
          resolver({data: response, meta: action.meta}, 'User', 'findSuccess'),
